...
func newEvent(et int, c interface{}, data interface{}) *model.EventItem {
...
func (c *GtaConnection) SetId(id int) {
	c.id = id
}

func (c *GtaConnection) GetId() int {
	return c.id
}
...
func (c *GtaConnection) pushEvent(et int, data interface{}) {
	...
	c.eventQueue.Push(newEvent(et, c, data))
}

func (c *GtaConnection) processSend() error {
	defer func() {
		c.logger.LogInfo("Connection Id: %d, Send Process End", c.id)
	}()
...
func (c *GtaConnection) processRecv() error {
	defer func() {
		c.logger.LogInfo("Connection Id: %d, Recv Process End", c.id)
	}()
